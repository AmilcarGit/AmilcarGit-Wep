...
const handler = async (m, { conn, usedPrefix: _p }) => {
  try {
    ...
    const help = Object.values(global.plugins || {})
      .filter(p => !p.disabled && p.tags && p.tags.includes('tools'))
      .map(p => ({
    ...
    const totalTools = help.length
    const totalPlugins = Object.keys(global.plugins || {}).length

    const comandosTools = help.map(menu => {
 ...
 ‧̍̊·̊‧̥°̩̥˚̩̩̥͙°̩̥‧̥·̊‧̍̊ ♡ °̩̥˚̩̩̥͙°̩̥ ·͙*̩̩͙˚̩̥̩̥*̩̩̥͙·̩̩̥͙*̩̩̥͙˚̩̥̩̥*̩̩͙‧͙ °̩̥˚̩̩̥͙°̩̥ ♡ ‧̍̊·̊‧̥°̩̥˚̩̩̥͙°̩̥‧̥·̊‧̍̊
  ...
  ☢︎ *HERRAMIENTAS DISPONIBLES:*
  🛠️ Comandos tools: ${totalTools}
  📦 Plugins totales: ${totalPlugins}
...
`
    ...
    console.error('💥 Error en menú herramientas:', e)
    await conn.reply(m.chat, `❌ Ocurrió un error al cargar el menú de herramientas.`, m)
  }
}

handler.command = ['menutools', 'tools', 'herramientas']
handler.tags = ['main']
